// Claude API proxy for MULTI-TURN chat — used by the Quick Tools interactive runner.
//
// This is the sibling of /api/generate/route.ts. The difference: /api/generate sends
// ONE user message; this route relays a whole conversation (an array of user/assistant
// turns) so the interactive modes (#4 Socratic, #9 Plan, #20 Guided) can hold a real
// back-and-forth. Everything else — the SSE framing, the [DONE] sentinel, the provider
// branches, the key checks, the headers — is identical, so the browser-side reader
// (lib/tools/chat.ts) parses it exactly like callAgent parses /api/generate.
//
//   Browser → POST /api/tools-chat → this route → Claude API → SSE stream back to browser
...
// One conversation turn, mirroring the Anthropic/OpenAI message shape.
...
interface ToolsChatRequest {
  systemPrompt: string          // the ARS SKILL system prompt (from /lib/ars-agents/)
  messages: ChatMessage[]       // the full conversation so far (user/assistant turns)
...
// Validate that messages is a non-empty array of well-formed user/assistant turns.
...
export async function POST(req: NextRequest) {
  try {
    let body: ToolsChatRequest
    try {
      body = (await req.json()) as ToolsChatRequest
    } catch {
      return new Response(
    ...
    if (!systemPrompt || !isValidMessages(messages)) {
      return new Response(
        JSON.stringify({ error: 'systemPrompt and a non-empty messages[] (role/content) are required' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      )
    }
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          console.error('[/api/tools-chat] stream error:', err)
          send({ error: message })
        } finally {
          controller.close()
        }
    ...
    console.error('[/api/tools-chat] error:', err)
    return new Response(
